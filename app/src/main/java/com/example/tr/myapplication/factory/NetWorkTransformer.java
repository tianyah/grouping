...
import com.example.tr.myapplication.api.APIException;
 ...
 * @param <T>
 */
public class NetWorkTransformer<T> implements ObservableTransformer<ApiResponse<T>, T> {
     ...
     */
    private Observable<T> flatResponse(final ApiResponse<T> response) {
        return Observable.just(response)
                .map(new Function<ApiResponse<T>, T>() {
                    @Override
                    public T apply(@NonNull ApiResponse<T> tBaseResponse) throws Exception {
                        //拿到后台返回code
                        ...
                        if (NetWorkManager.isOpenApiException()) {
                            errorMsg = tBaseResponse.getMessage();
                        }
                        //抛出异常,走到onError.
    ...
    public ObservableSource<T> apply(Observable<ApiResponse<T>> upstream) {
        return upstream
                ...
                .flatMap(new Function<ApiResponse<T>, ObservableSource<T>>() {
                    @Override
                    public ObservableSource<T> apply(ApiResponse<T> tBaseResponse) throws Exception {
